  ...
  return (
    <div className={`bg-black min-h-screen relative`}>
      ...
      <div
        ...
        className="w-[335px] flex flex-col gap-[8px] h-fit pb-5 mx-auto mt-[10px] rounded-[15px] border bg-[#0D1015] border-[#232429]"
      >
        ...
        <div className="h-[38px] p-[10px] flex ml-[18px] mr-[18px] items-center gap-[10px] rounded-[8px] border border-[#232429] relative">
          <div className="flex items-center gap-2 cursor-pointer h-full" onClick={() => setIsOpen(!isOpen)}>
            <span className={`fi fi-${currentFlag?.id} w-[20px] h-[15px]`}></span>
            <svg
              ...
              <ul className="absolute left-0 top-[calc(100%+8px)] w-full max-h-[160px] overflow-y-auto bg-[#0D1015] border border-[#232429] rounded-[8px] z-50 py-1 shadow-xl">
                {FLAGS.map((flag) => (
                  <li
                    ...
                    className="px-3 py-2 hover:bg-[#232429] cursor-pointer flex items-center gap-3"
                  >
                    <span className={`fi fi-${flag.id} w-[18px] h-[13px]`}></span>
                    <span className="text-[#989AA3] text-[13px]">{flag.name}</span>
                  </li>
                ))}
              </ul>
            )}
